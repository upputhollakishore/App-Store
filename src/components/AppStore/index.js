...
class AppStore extends Component {
  ...
  render() {
    const {activeTabId, searchItem} = this.state
    const filteredProjects = this.getFilteredList()
    const searchProjects = filteredProjects.filter(eachInputItem =>
      eachInputItem.appName.toLowerCase().includes(searchItem.toLowerCase()),
    )

    return (
      <div className="bg-container">
        <div className="bottom-container">
          <h1 className="heading-store">App Store</h1>
          ...
          <ul className="app-container">
            {searchProjects.map(eachAppItem => (
